...
class MutationNetwork:
    ...
    def create_plot(self, mutation_df, all_mutation_matrix):
        ...
        try:
            ...
            plt.title("Network of Co-occurring Mutations", fontsize=16, fontweight='bold') # write title
            plt.axis("off")
            plt.tight_layout()
            plt.savefig("output/mutation_network.png", dpi=300) # save to output
            print("Figure 2: mutation network plot")
            
            return co_occurrence_matrix # return matrix if needed
            
        except Exception as e:
            print(f"Error in network plot: {e}") # catching any errors, if needed
            return None